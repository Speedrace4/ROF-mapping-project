...
var overlayPopup2 = new ol.Overlay({
    element: container2
});
...
var expandedAttribution2 = new ol.control.Attribution({
    collapsible: false
});
...
map.getView().fit([-9798650.124116, 5090505.780824, -9721601.855923, 5169624.836970], map.getSize());

var map2 = new ol.Map({
    controls: ol.control.defaults({attribution:false}).extend([
        expandedAttribution2
    ]),
    target: document.getElementById('map2'),
    renderer: 'canvas',
    overlays: [overlayPopup2],
    layers: layersList2,
    view: new ol.View({
         maxZoom: 28, minZoom: 1
    })
...
var onPointerMove = function(map, featureOverlay, container, closer, overlayPopup, content, evt) {
...
var onSingleClick = function(map, container, closer, overlayPopup, content, evt) {
    if (doHover) {
        return;
    }
    if (sketch) {
...
map.on('pointermove', function(evt) {
    onPointerMove(map, featureOverlay, container, closer, overlayPopup, content, evt);
});
map.on('singleclick', function(evt) {
    onSingleClick(map, container, closer, overlayPopup, content, evt);
});

map2.on('pointermove', function(evt) {
    onPointerMove(map2, featureOverlay2, container2, closer2, overlayPopup2, content2,  evt);
});
map2.on('singleclick', function(evt) {
    onSingleClick(map2, container2, closer2, overlayPopup2, content2,  evt);
});
...
map2.on("rendercomplete", function(evt) {
    if (!attributionComplete) {
        var attribution = document.getElementsByClassName('ol-attribution')[0];
        var attributionList = attribution.getElementsByTagName('ul')[0];
        var firstLayerAttribution = attributionList.getElementsByTagName('li')[0];
